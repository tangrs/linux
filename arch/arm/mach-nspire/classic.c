...
asmlinkage void __exception_irq_entry
    ...
    do {
        ...
        serviced += check_interrupt(reg_base, regs);            /* IRQ */
        serviced += check_interrupt(reg_base + 0x100, regs);    /* FIQ */
    } while (serviced > 0);
}
...
static void __init classic_allocate_gc(void) {
	...
	gc = irq_alloc_generic_chip("NINT", 1, 0, IOMEM(NSPIRE_INTTERUPT_VIRT_BASE),
	    handle_level_irq);
...
static void classic_timer_set_mode(enum clock_event_mode mode,
	...
	evt->mode = mode;
	printk("Timer mode set to %d, evt->mode = %d\n", mode, evt->mode);
}
...
static irqreturn_t classic_timer_interrupt(int irq, void *dev_id) {
    ...
    if (c->event_handler) c->event_handler(c);
...
static void __init classic_timer_init(void) {
    ...
    printk("Clock source registered\n");

    clockevents_config_and_register(&nspire_clkevt, clk_get_rate(timer_clk),
        0x0001, 0xfffe);
    printk("Clock events registered\n");
}
